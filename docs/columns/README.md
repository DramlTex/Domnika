...
`COUNTRY_ORDER` и `TYPE_ORDER` в `table.js` определяют приоритет стран и типов. Отсутствующие в массивах значения сортируются по алфавиту. При смене вкладки или применении фильтров вызывается `fillTable` — данные выводятся заново уже с учётом выбранных условий.
