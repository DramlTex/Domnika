...
базы и число строк в отчёте `stock/bystore/current?include=zeroLines`. При пропуске элемента из-за
отсутствия данных или неподходящей группы в лог заносится соответствующее
сообщение. Это помогает отладить причину пустых таблиц.
